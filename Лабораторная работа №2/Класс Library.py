...
class Book:
    def __init__(self, id: int, name: int, pages: int):
        self.id = id
        self.name = name
        self.pages = pages
...
class Library:
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}({self.books})"
    def get_next_book_id(self):
        if self.books is None:
            return 1
        else:
            return self.books[-1].__dict__.get('id') + 1
    def get_index_by_book_id(self, id):

       for i in range(len(self.books)):
           if id == self.books[i].__dict__.get('id'):
               return i
           else:
